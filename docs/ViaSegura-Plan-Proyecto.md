...
|------|--------------------------------|
| Fin de Semana 1 | Los tres módulos tienen su esqueleto corriendo localmente (app, dashboard, API, microservicio) y cada uno hizo su commit de setup en la rama `feature/moduloX-*-semana1`. |
...
| Fin de Semana 3 (Feature Freeze) | Cada módulo publica su Release Candidate; no se agregan features nuevas después de este punto, solo corrección de bugs vía ramas `fix/moduloX-*`. |
...
**Paso 1 — Nomenclatura de ramas.** Cada rama de trabajo sigue el patrón `feature/moduloX-nombre-semanaY` (ej. `feature/modulo2-backend-semana2`). En la Semana 3, los fixes de bugs usan `fix/moduloX-descripcion-corta` en vez de reabrir la rama de feature, para dejar claro en el historial qué se corrigió y por qué.
